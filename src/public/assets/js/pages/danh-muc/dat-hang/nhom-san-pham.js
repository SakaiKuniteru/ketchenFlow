...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initialize() {
        try {
            ...
            console.error('Không thể khởi tạo danh mục nhóm sản phẩm.', error);

            window.MCS?.toast?.error(error?.message || 'Không thể tải danh mục nhóm sản phẩm.');
        }
    }
        ...
        catalog = await window.MCS.pages.createCatalogPage({
            ...
            validation: {
                maNhomSanPham: {
                    ...
                    maxLength: 50,
                    unique: true,
                    requiredMessage: 'Vui lòng nhập mã nhóm sản phẩm.',
                    maxLengthMessage: 'Mã nhóm sản phẩm không được vượt quá 50 ký tự.',
                    uniqueMessage: 'Mã nhóm sản phẩm đã tồn tại.'
                },
                ...
                tenNhomSanPham: {
                    ...
                    maxLength: 150,
                    unique: true,
                    requiredMessage: 'Vui lòng nhập tên nhóm sản phẩm.',
                    maxLengthMessage: 'Tên nhóm sản phẩm không được vượt quá 150 ký tự.',
                    uniqueMessage: 'Tên nhóm sản phẩm đã tồn tại.'
                },

                loaiSanPham: {
                    label: 'Loại sản phẩm',
                    required: true,
                    requiredMessage: 'Vui lòng chọn loại sản phẩm.'
                },
            ...
            mapListResponse(result) {
                ...
                return danhSach.map((record) => ({
                    ...record,
                    loaiSanPhamText: getLoaiSanPhamLabel(record.loaiSanPham)
                }));
            },
            ...
            transformPayload(formData) {
                return {
                    maNhomSanPham: String(formData.maNhomSanPham || '')
                        .trim()
                        .toUpperCase(),

                    tenNhomSanPham: String(formData.tenNhomSanPham || '').trim(),
                    ...
                    thuTuHienThi: parseThuTuHienThi(formData.thuTuHienThi),
            ...
            validate(formData) {
                ...
                const loaiHopLe = dsLoaiSanPham.some((item) => Number(item.value) === formData.loaiSanPham);

                if (formData.loaiSanPham !== null && !loaiHopLe) {
                    errors.loaiSanPham = 'Loại sản phẩm không hợp lệ.';
                }
                ...
                if (
                    ...
                    errors.thuTuHienThi = 'Thứ tự hiển thị phải là số nguyên từ 0 đến 2.147.483.647.';
                }
    ...
    async function loadLoaiSanPham() {
        const result = await window.MCS.api.request('/api/mcs/v1/enums?name=loaiSanPham');

        dsLoaiSanPham = Array.isArray(result?.data) ? result.data : [];

        if (dsLoaiSanPham.length === 0) {
            throw new Error('Chưa có dữ liệu loại sản phẩm.');
        }
    ...
    function getLoaiSanPhamLabel(value) {
        const item = dsLoaiSanPham.find((option) => Number(option.value) === Number(value));

        return item?.name || String(value ?? '—');
    }

    function parseThuTuHienThi(value) {
        const normalized = window.MCS.numberInput.normalizeValue(value);
    ...
    async function exportData() {
        try {
            ...
            window.MCS?.toast?.success('Xuất dữ liệu nhóm sản phẩm thành công.');
        } catch (error) {
            console.error('Xuất dữ liệu nhóm sản phẩm thất bại.', error);
        ...
        input.addEventListener(
            ...
            async () => {
                ...
                try {
                    ...
                    console.error('Import nhóm sản phẩm thất bại.', error);
