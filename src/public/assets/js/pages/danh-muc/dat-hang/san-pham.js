...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function bindNumberFields() {
        for (const [id, rule] of Object.entries(NUMBER_RULES)) {
            ...
            input.addEventListener("paste", event => {
                ...
                const pattern = rule.integer ? /^(?:\d+|\d{1,3}(?:\.\d{3})+)$/ :
                    /^(?:\d+|\d{1,3}(?:\.\d{3})+)(?:,\d*)?$/;
                if (!pattern.test(text)) {
                    event.preventDefault();
                    window.MCS.toast?.error(rule.integer ? `${rule.label} chỉ nhận số nguyên.` :
                        "Dùng dấu chấm hàng nghìn và dấu phẩy thập phân.");
                }
            });
            // Capture: xử lý trước listener input của component chung.
            input.addEventListener("input",
                () => formatEnteredNumber(input, rule), true);
        }
    }
    ...
    function formatEnteredNumber(input, rule) {
        ...
        let whole = (commaIndex < 0 ? raw : raw.slice(0, commaIndex)).replace(/\D/g, "").replace(/^0+(?=\d)/,
            "").slice(0, 12);
        let value;
        if (rule.integer) {
            ...
            const fraction = commaIndex < 0 ? null : raw.slice(commaIndex + 1).replace(/\D/g, "").slice(0, 5);
    ...
    function transformPayload(f) {
        ...
        return {
            maSanPham: String(f.maSanPham || "").trim().toUpperCase(),
    ...
    function validateForm(f) {
        const errors = {};
        if (!Number.isSafeInteger(f.nhomSanPhamId) || f.nhomSanPhamId <= 0 || !dsNhomSanPham.some(item =>
                String(item.id) === String(f.nhomSanPhamId))) {
            errors.nhomSanPhamId = "Vui lòng chọn nhóm sản phẩm đang hoạt động.";
        }
        if (f.donViTinhId !== null && f.donViTinhId !== "" && (!Number.isSafeInteger(f.donViTinhId) || f
                .donViTinhId <= 0 || !dsDonViTinh.some(item => String(item.id) === String(f.donViTinhId)))) {
            errors.donViTinhId = "Đơn vị tính không hợp lệ hoặc đã ngừng hoạt động.";
        }
        if (!/^(?:0|[1-9]\d{0,11})(?:\.\d{1,5})?$/.test(f.giaBan)) {
            errors.giaBan = "Giá bán phải không âm, tối đa 12 chữ số phần nguyên " +
            "và 5 chữ số sau dấu phẩy.";
        }
        for (const [id, rule] of Object.entries(NUMBER_RULES)) {
            if (!rule.integer) {
                continue;
    ...
    function importData(catalogInstance) {
        ...
        input.addEventListener("change", async () => {
            ...
            try {
                ...
                window.MCS.api.downloadBlob(result.blob, result.fileName || FILE_NAME);
                window.MCS.toast?.success("Đã xử lý import. Vui lòng xem file kết quả " +
                    "để biết các dòng thành công hoặc lỗi.");
                try {
