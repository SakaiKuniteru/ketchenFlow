...
const giaTriLoaiGiaoDich = dsLoaiGiaoDich.map((item) => Number(item.value));
...
const huyQrSchema = Joi.object({
    noiDung: Joi.string().trim().max(500).allow('', null).optional()
});
...
const hoanTienSchema = Joi.object({
    thanhToanId: Joi.number().integer().positive().required().messages({
    ...
    soLuongHoan: Joi.number().integer().min(1).required().messages({
    ...
    lyDoHoan: Joi.string().trim().max(500).required(),
...
const callbackSchema = Joi.object({
    ...
    trangThai: Joi.number().integer().required(),

    maThamChieu: Joi.string().trim().max(100).allow('', null).optional(),

    maChuanChi: Joi.string().trim().max(100).allow('', null).optional(),

    noiDungLoi: Joi.string().trim().max(1000).allow('', null).optional()
}).unknown(true);
