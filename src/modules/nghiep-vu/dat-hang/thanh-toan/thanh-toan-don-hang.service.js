...
class ThanhToanService {
    async create(donHangId, user) {
        ...
        if (order.trang_thai_thanh_toan === TRANG_THAI_THANH_TOAN.DA_THANH_TOAN)
            throw new ApiError(409, 'Đơn hàng đã được thanh toán.');
    ...
    async confirm(id, data, user) {
        ...
        try {
            ...
            await client.query(`UPDATE nv_don_hang SET trang_thai_thanh_toan = $2, updated_at = NOW() WHERE id = $1`, [
