...
const { MA_BAO_CAO, HEADER_ROW, DATA_START_ROW } = require('./nhom-san-pham.export');
...
function parseNonNegativeInteger(value, fieldName) {
    ...
    if (number !== undefined && number < 0) {
        throw new ApiError(400, `${fieldName} không được nhỏ hơn 0.`);
    }
...
function readItem(row, rowNumber, getValue, keyConfig) {
    const codeField = keyConfig.hasCodeKey ? 'maNhomSanPham/k' : 'maNhomSanPham';
    ...
    return {
        ...
        maNhomSanPham: getValue(row, codeField),

        tenNhomSanPham: getValue(row, 'tenNhomSanPham'),

        loaiSanPham: getValue(row, 'loaiSanPham'),
...
function createBusinessData(item) {
    ...
    if (item.tenNhomSanPham !== undefined) {
        data.tenNhomSanPham = item.tenNhomSanPham;
    }

    const loaiSanPham = parseInteger(item.loaiSanPham, 'Loại sản phẩm');

    if (loaiSanPham !== undefined) {
...
async function processItem(item) {
    if (item.id !== undefined) {
        item.id = parsePositiveInteger(item.id, 'ID nhóm sản phẩm');
    }

    const strategy = await resolveImportStrategy(item, {
        getById: (id) => nhomSanPhamRepository.getChiTiet(id),

        getByCode: (code) => nhomSanPhamRepository.getChiTietByMa(code),

        getRecordCode: (record) => record.maNhomSanPham,
    ...
    if (strategy.action === 'UPDATE') {
        if (
            ...
            item.maNhomSanPham !== undefined &&
            shouldChangeCode(item.maNhomSanPham, strategy.record.maNhomSanPham)
        ) {
            data.maNhomSanPham = item.maNhomSanPham;
        }
        ...
        const result = await nhomSanPhamService.update(strategy.record.id, data);
        ...
        return {
            ...
            maNhomSanPham: result.maNhomSanPham,
    ...
    if (!item.maNhomSanPham) {
        throw new ApiError(400, 'Thêm mới nhóm sản phẩm phải có mã nhóm sản phẩm.');
    }

    data.maNhomSanPham = item.maNhomSanPham;

    const result = await nhomSanPhamService.create(data);
...
async function importNhomSanPham(file) {
    ...
    const keyConfig = validateKeyHeaders(headerMap, {
        ...
        codeKey: 'maNhomSanPham/k',

        codeField: 'maNhomSanPham'
    });
