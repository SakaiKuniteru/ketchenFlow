...
function mapExportItem(item) {
    return {
        ...
        choPhepDat: item.choPhepDat,
        laSanPhamMoi: item.laSanPhamMoi,
        laSanPhamNoiBat: item.laSanPhamNoiBat,
        soLuongToiThieu: item.soLuongToiThieu,
        soLuongToiDa: item.soLuongToiDa,
        buocSoLuong: item.buocSoLuong,
        thoiGianChuanBiPhut: item.thoiGianChuanBiPhut,
        thuTuHienThi: item.thuTuHienThi,
        active: item.active
    };
}
async function exportSanPham(query = {}) {
    const danhSach = await sanPhamRepository.getTongHop(query);
