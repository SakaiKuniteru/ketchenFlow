...
async function save(data, file, id = null) {
    ...
    try {
        ...
        await client.query('SELECT pg_advisory_lock(hashtext($1))', ['dm-san-pham-images']);
        locked = true;
        ...
        const result = id === null ? await repository.create(data, client) : await repository.update(id, data, client);
        if (!result) {
            throw new ApiError(404, 'Sản phẩm không tồn tại.');
        }
        ...
        if (locked) {
            try {
                await client.query('SELECT pg_advisory_unlock(hashtext($1))', ['dm-san-pham-images']);
            } catch (error) {
