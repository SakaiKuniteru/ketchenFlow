...
const voucherDonHangService = require('./voucher-don-hang.service');

const { MA_BAO_CAO, HEADER_ROW, DATA_START_ROW } = require('./voucher-don-hang.export');

const numberFields = {
    loaiGiam: 'Loại giảm',
    giaTri: 'Giá trị giảm',
    giamToiDa: 'Giảm tối đa',
    giaTriDonHangToiThieu: 'Giá trị đơn hàng tối thiểu',
    soLuongPhatHanh: 'Số lượng phát hành',
    soLuotMoiNhanVien: 'Số lượt mỗi nhân viên',
    phamViApDung: 'Phạm vi áp dụng'
};

const integerFields = new Set(['loaiGiam', 'soLuongPhatHanh', 'soLuotMoiNhanVien', 'phamViApDung']);
...
function parseNumberField(value, fieldName, integer = false) {
    ...
    if (integer && !Number.isInteger(number)) {
        throw new ApiError(400, `${fieldName} phải là số nguyên.`);
    }
...
function parseIdList(value, fieldName) {
    ...
    if (ids.some((id) => !Number.isInteger(id) || id <= 0)) {
        throw new ApiError(400, `${fieldName} phải là danh sách ID nguyên dương, phân cách bằng dấu phẩy.`);
    }
...
function readItem(row, rowNumber, getValue, keyConfig) {
    const codeField = keyConfig.hasCodeKey ? 'maVoucher/k' : 'maVoucher';
    ...
    const item = {
        ...
        maVoucher: getValue(row, codeField),

        tenVoucher: getValue(row, 'tenVoucher'),

        moTa: getValue(row, 'moTa'),

        thoiGianBatDau: getValue(row, 'thoiGianBatDau'),

        thoiGianKetThuc: getValue(row, 'thoiGianKetThuc')
    };

    for (const key of Object.keys(numberFields)) {
        item[key] = getValue(row, key);
    }
...
function createBusinessData(item) {
    ...
    for (const key of ['tenVoucher', 'moTa']) {
        if (item[key] !== undefined) {
            data[key] = item[key];
        }
    }

    for (const [key, fieldName] of Object.entries(numberFields)) {
        const value = parseNumberField(item[key], fieldName, integerFields.has(key));
    ...
    for (const key of booleanFields) {
        const value = parseBooleanField(item[key], key);
    ...
    for (const key of relationFields) {
        const value = parseIdList(item[key], key);
    ...
    const thoiGianBatDau = parseDateTime(item.thoiGianBatDau, 'Thời gian bắt đầu');

    if (thoiGianBatDau !== undefined) {
        data.thoiGianBatDau = thoiGianBatDau;
    }

    const thoiGianKetThuc = parseDateTime(item.thoiGianKetThuc, 'Thời gian kết thúc');
...
async function processItem(item, user) {
    if (item.id !== undefined) {
        item.id = parsePositiveInteger(item.id, 'ID voucher đơn hàng');
    }

    const strategy = await resolveImportStrategy(item, {
        getById: (id) => voucherDonHangRepository.getChiTiet(id),

        getByCode: (code) => voucherDonHangRepository.getChiTietByMa(code),

        getRecordCode: (record) => record.maVoucher,
    ...
    if (strategy.action === 'UPDATE') {
        if (
            ...
            shouldChangeCode(item.maVoucher, strategy.record.maVoucher)
        ) {
            data.maVoucher = item.maVoucher;
        }
        ...
        const result = await voucherDonHangService.update(strategy.record.id, data);
    ...
    if (!item.maVoucher) {
        throw new ApiError(400, 'Thêm mới voucher đơn hàng phải có mã voucher.');
    }

    data.maVoucher = item.maVoucher;

    const result = await voucherDonHangService.create(data, user);
...
async function importVoucherDonHang(file, user) {
