...
function formatNumberByPattern(value, pattern) {
    ...
    if (lastComma >= 0 && lastDot >= 0) {
        ...
        if (separator) {
            ...
            const fractionPattern = format.slice(index + 1).replace(/[^#0]/g, '');

            if (fractionPattern.length === 3 && !fractionPattern.includes('0')) {
    ...
    if (decimalSeparator) {
        const fractionPattern = format.slice(format.lastIndexOf(decimalSeparator) + 1).replace(/[^#0]/g, '');

        maximumFractionDigits = fractionPattern.length;

        minimumFractionDigits = (fractionPattern.match(/0/g) || []).length;
    }

    let [integerPart, fractionPart = ''] = Math.abs(number).toFixed(maximumFractionDigits).split('.');
    ...
    if (fractionPart) {
        return sign + integerPart + (decimalSeparator || '.') + fractionPart;
    }
...
function formatValue(value, pattern) {
    ...
    if (isDateValue(value)) {
        return formatDateByPattern(value, pattern);
    }

    if (isNumberValue(value)) {
        return formatNumberByPattern(value, pattern);
    }
...
function isKnownDateTimePattern(pattern) {
    return DINH_DANG_NGAY_GIO.includes(String(pattern || '').trim());
}
